...
""" Adapted from https://github.com/dbolya/yolact/tree/master/utils. """
...
def disable(fn_name):
	""" Disables the given function name from being considered for the average or outputted in print_stats. """
...
def save_time_stats_to_json(json_path):
	""" Get the current timing information into a dict. """
	_time_stats = {}

	all_fn_names = [k for k in _total_times.keys() if k not in _disabled_names]

	for name in all_fn_names:
		_time_stats.update({name: _total_times[name]*1000})
	
	_time_stats.update({'Total': total_time()*1000})

	import os
	import pandas as pd
	exp_name = os.path.basename(json_path)
	pd.DataFrame({exp_name:_time_stats}).to_json(json_path+'_time.json', indent=2)
