...
class SavePath:
    ...
    def from_str(path:str):
        ...
        if file_name.endswith('interrupt') or file_name.endswith('best'):
            params = params[:-1]
    ...
    @staticmethod
    def remove_best(save_folder):
        for p in Path(save_folder).glob('*_best.pth'):
            p.unlink()
    
    @staticmethod
    def get_best(save_folder):
        for p in Path(save_folder).glob('*_best.pth'): 
            return str(p)
        return None
